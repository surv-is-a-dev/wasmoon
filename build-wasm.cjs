...
function execAndLink(useExecFile, cmd_or_file, args_or_options, options_or_none) {
  return new Promise((resolve, reject) => {
    if (useExecFile) {
      return execFile(cmd_or_file, args_or_options ?? [], options_or_none ?? {}, (error) => {
        if (error) {
          return reject(error);
        }
...
const luaSrc = Array.from(fs.globSync('./lua/*.c')).filter((f) => {
  f = path.basename(String(f));
  return !(f.includes('luac.c') || f.includes('lua.c'));
});
